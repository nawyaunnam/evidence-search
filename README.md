# EvidenceSearch — retrieval with source citations

A BM25 document retrieval engine that returns traceable excerpts and evaluates ranking quality with recall@k and reciprocal rank.

**Focus:** AI engineering / retrieval evaluation / information retrieval · Python 3.11+ · Standard library · Offline demo
...
| Retrieval | BM25 ranking, inverted index, overlapping text chunks |
| Evaluation | Recall@k and mean reciprocal rank |
...
Indexes the current [USGS seven-day earthquake feed](https://earthquake.usgs.gov/earthquakes/feed/v1.0/geojson.php) and returns source-linked evidence. Feed descriptions are real observations. This is a retrieval engine; it does not invent prose answers or call an LLM.
...
Collect independent relevance judgments and compare BM25 with an embedding retriever on the same held-out queries.
